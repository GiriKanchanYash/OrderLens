...
class Config:
    ...
    Resolution priority for secrets:
      ...
      4. _vault.get("FABRIC_ORDERLENS_DATABASE", _settings.get("fabric", {}).get("database", "OrderLens"),)
    ...
    """
    FABRIC_ORDERLENS_DATABASE: str = _settings.get(
        "fabric", {}).get("database", "OrderLens")
    """
    FABRIC_ORDERLENS_DATABASE: str = _vault.get(
        "FABRIC_ORDERLENS_DATABASE",
        _settings.get("fabric", {}).get("database", "OrderLens")
    )
    """
    ...
    FABRIC_ORDERLENS_WAREHOUSE_DATABASE: str = _vault.get(
        "FABRIC_WAREHOUSE_DATABASE",
        _settings.get("fabric", {}).get("warehouse_database", "DW_OrderLens"),
    )
    """
    FABRIC_ORDERLENS_WAREHOUSE_DATABASE: str = _vault.get(
        "FABRIC_ORDERLENS_WAREHOUSE_DATABASE",
        _settings.get("fabric", {}).get("warehouse_database", "DW_OrderLens")
    )
    # Schema names
    SCHEMA: str = _settings.get("fabric", {}).get("schema", "INFORMATION_MART")
    DEFAULT_SCHEMA: str = _settings.get(
        "fabric", {}).get("default_schema", "dbo")
    ...
    APP_TITLE: str = _ui.get("app_title", "OrderLens Analytics")
    PAGE_ICON_URL: str = _ui.get("page_icon_url", "")
    DEFAULT_BG_COLOR: str = _ui.get("default_bg_color", "#FBF9F4")
    BRAND_NAME: str = _ui.get("brand_name", "OrderLens")
    ...
    SCHEMA_METADATA_FILE: str = _settings.get(
        "metadata", {}
    ).get("schema_file", "schema_model.yaml")

    # Data Vault layer names used in YAML generation (req 5, 7)
    DATA_VAULT_HUB_PREFIX: str = _settings.get(
        "data_vault", {}).get("hub_prefix", "h_")
    DATA_VAULT_SAT_PREFIX: str = _settings.get(
        "data_vault", {}).get("sat_prefix", "s_")
    DATA_VAULT_LINK_PREFIX: str = _settings.get(
        "data_vault", {}).get("link_prefix", "l_")
    DATA_VAULT_BRIDGE_PREFIX: str = _settings.get(
        "data_vault", {}).get("bridge_prefix", "BRG_")
    ...
    HISTORY_TABLE_NAME: str = _naming.get(
    ...
    def validate(cls) -> bool:
        ...
        for attr, label in [
            ...
            ("FABRIC_ORDERLENS_DATABASE",        "FABRIC_ORDERLENS_DATABASE"),
    ...
    def validate_connection_values(cls) -> list[str]:
        ...
        lines.append(
            f"FABRIC_SQL_SERVER        : {mask(cls.FABRIC_SQL_SERVER)}")
        lines.append(
            f"FABRIC_ORDERLENS_DATABASE          : {cls.FABRIC_ORDERLENS_DATABASE or '(NOT SET)'}")
        lines.append(
            f"FABRIC_WAREHOUSE_SERVER  : {mask(cls.FABRIC_WAREHOUSE_SERVER)}")
        lines.append(
            f"FABRIC_WAREHOUSE_DATABASE: {cls.FABRIC_WAREHOUSE_DATABASE or '(NOT SET)'}")
        lines.append(f"AZURE_TENANT_ID          : {mask(cls.AZURE_TENANT_ID)}")
        lines.append(f"AZURE_CLIENT_ID          : {mask(cls.AZURE_CLIENT_ID)}")
        lines.append(
            f"AZURE_CLIENT_SECRET      : {mask(cls.AZURE_CLIENT_SECRET)}")
        lines.append(
            f"AZURE_OPENAI_ENDPOINT    : {cls.AZURE_OPENAI_ENDPOINT or '(NOT SET)'}")
        lines.append(
            f"AZURE_OPENAI_DEPLOYMENT  : {cls.AZURE_OPENAI_DEPLOYMENT or '(NOT SET)'}")
        return lines
    ...
    def print_diagnostics(cls) -> None:
        ...
        missing = [
            name for name, attr in [
                ...
                ("FABRIC_ORDERLENS_DATABASE",     cls.FABRIC_ORDERLENS_DATABASE),
        ...
        if missing:
            ...
            print(
                "  Check your .env file - values must NOT have surrounding quotes on Windows.")
    ...
    def get_connection_string(cls) -> str:
        return (
            ...
            f"Database={cls.FABRIC_ORDERLENS_DATABASE};"
            f"Authentication=ActiveDirectoryServicePrincipal;"
            # f"Authentication=ActiveDirectoryPassword;"
            f"UID={cls.AZURE_CLIENT_ID};"
            f"PWD={cls.AZURE_CLIENT_SECRET};"
            # f"UID=sravan.devarasetty@yashtechnologies841.onmicrosoft.com;"
            # f"PWD=Welcome0226%$#ert;"
            f"Encrypt=yes;"
    ...
    def get_warehouse_connection_string(cls) -> str:
        return (
            f"Driver={{ODBC Driver 18 for SQL Server}};"
            f"Server={cls.FABRIC_SQL_SERVER};"
            f"Database={cls.FABRIC_ORDERLENS_WAREHOUSE_DATABASE};"
            f"Authentication=ActiveDirectoryServicePrincipal;"
            f"UID={cls.AZURE_CLIENT_ID};"
            f"PWD={cls.AZURE_CLIENT_SECRET};"
            f"Encrypt=yes;"
...
if __name__ == "__main__":
    ...
    print(_vault.get("FABRIC_SQL_SERVER", _settings.get(
        "fabric", {}).get("sql_server", "")))
    print(_settings.get(
        "fabric", {}).get("database", "OrderLens"))
    print(os.getenv("AZURE_KEY_VAULT_URL", ""))
    print(_vault.get("AZURE_TENANT_ID", ""))
    print(_vault.get("AZURE_CLIENT_ID", ""))
    print(_vault.get("AZURE_CLIENT_SECRET", ""))
"""
